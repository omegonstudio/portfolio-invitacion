...
export async function POST(
  ...
  try {
    ...
    const existingRsvp = rsvpDatabase.find(
      (rsvp) => rsvp.name.toLowerCase() === body.name.toLowerCase()
    );
    ...
    return NextResponse.json(
      {
        success: true,
        message: `¡Gracias ${body.name}! Tu confirmación para ${
          body.guests
        } persona${
          Number.parseInt(body.guests) > 1 ? "s" : ""
...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    const totalGuests = rsvpDatabase.reduce(
      (sum, rsvp) => sum + Number.parseInt(rsvp.guests),
      0
    );
    ...
    const stats = {
      ...
      averageGuestsPerRsvp:
        totalRsvps > 0 ? (totalGuests / totalRsvps).toFixed(1) : 0,
